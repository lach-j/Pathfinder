...
test("creates workstreams with markdown and JSON state files", async () => {
    ...
    assert.deepEqual(await sortedFiles(path.join(repo, ".pathfinder", "workstreams", workstream.id)), ["comments.json", "plan.md", "pr.md", "reviews.json", "slices.json", "workstream.json"]);
});
...
async function sortedFiles(directory) {
